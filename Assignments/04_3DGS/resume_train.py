...
class GaussianTrainer:
    ...
    def compute_projection(
        ...
        # covs_cam = R @ covs3d @ R^T for each covariance matrix
    ...
    def train(self, train_loader: DataLoader, start_epoch: int = 0):
        """Main training loop"""
        ...
        for epoch in range(start_epoch, start_epoch + self.config.num_epochs):  # 修改: 引入起始 epoch
            # Training loop
...
def parse_args():
    ...
    parser.add_argument('--resume', type=str, default=None,
                      help='Path to checkpoint to resume from')  # 修改: 添加 resume 参数
    
    # Training parameters
    parser.add_argument('--num_epochs', type=int, default=100,
                      help='Number of training epochs to run')  # 修改: 表述更明确
...
def main():
    ...
    start_epoch = 0  # 修改: 初始化起始 epoch
    if args.resume:
        print(f"Resuming from checkpoint: {args.resume}")
        start_epoch = trainer.load_checkpoint(args.resume) + 1  # 修改: 加1以避免重复保存相同 epoch
    ...
    trainer.train(train_loader, start_epoch=start_epoch)  # 修改: 传递 start_epoch
